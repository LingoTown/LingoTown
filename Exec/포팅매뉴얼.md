...
### 3. EC2 사전 설정 (Docker Out Of Docker 방식 활용)
...
# Jenkins 컨테이너 실행
# 젠킨스 설치 및 실행 (23.10.18 부로 젠킨스의 lts가 업데이트되어 Java 17 이상 지원, 우분투가 아닌 데비안 기반 이미지를 사용해서 lts 이미지를 위 경우에 # # 해당될때만 쓰자
# 아래 경우에서는 23.09.20의 2.414.2 버전 활용 (Ubuntu 20.0.4LTS 기반))
# https://www.jenkins.io/changelog-stable/
$ sudo docker run -d -u root -it -v /var/run/docker.sock:/var/run/docker.sock -v /lib/modules:/lib/modules -p 8888:8080 --name jenkins jenkins/jenkins:2.414.2
```
...
// Jenkins 컨테이너 내부 필요 패키지 설치 및 사용자 권한 부여 (docker cli, vim)
apt-get update
