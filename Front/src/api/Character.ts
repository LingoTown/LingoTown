...
// 전체 캐릭터 정보 받아오기
const getCharacterList = async(success: (data: {data : ReturnType }) => void, fail: (error: unknown) => void) => {
...
const lockOffCharacter = async (payload: lockOff, success: ( data : { data : CommonResponseType }) => void, fail: (error: unknown) => void) => {
  ...
  export { getCharacterList, getCharacterLockInfo, updateCharacter, lockOffCharacter };