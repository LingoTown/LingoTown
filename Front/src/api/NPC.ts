...
const getMemberNpcRelationship = async (
  success: ({ data }: { data: ReturnType }) => void,
  fail: (error: unknown) => void
) => {
  await HttpJson.get(`api/talk/intimacy/list`).then(success).catch(fail);
};
