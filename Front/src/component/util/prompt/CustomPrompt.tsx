...
const CustomPrompt = ({ title, message, _default, onClickOK, onClickCancel }: PromptType) => {
  ...
  return (
    <div className="z-50" style={{ cursor: `url('/navigation_small.png'), auto` }}>
      ...
      <form className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 p-6 bg-white rounded-lg shadow-xl border border-gray-200" onSubmit={handleSubmit}>
        ...
        <div className="mt-6 flex justify-end space-x-2">
          <button style={{ cursor: `url('/navigation_hover_small.png'), auto` }} className="py-2 px-4 bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:ring focus:ring-blue-300 focus:ring-opacity-50 transition-colors" type="submit">
            OK
          </button>
          <button style={{ cursor: `url('/navigation_hover_small.png'), auto` }} onClick={onClickCancel} className="py-2 px-4 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 focus:ring focus:ring-gray-300 focus:ring-opacity-50 transition-colors" type="reset">
            Cancel
          </button>
        </div>
      </form>
