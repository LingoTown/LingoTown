...
const CustomAlert = ({ title, message, onClose }: AlertType) => {
  ...
  return (
    <div>
      <div
        style={{ zIndex:"2", cursor: `url('${import.meta.env.VITE_S3_URL}MousePointer/navigation_small.png'), auto`, fontFamily: "GabiaSolmee" }}
        className="fixed inset-0 bg-black bg-opacity-50 transition-opacity" onClickCapture={(e) => e.stopPropagation()}></div>
      <div
        style={{zIndex:"3", cursor: `url('${import.meta.env.VITE_S3_URL}MousePointer/navigation_small.png'), auto`, fontFamily: "GabiaSolmee" }}
